...
- 🎯 **红果短剧推荐** - 基于红果短剧数据，展示热门和高分短剧内容
- 🎌 **豆瓣动漫推荐** - 集成豆瓣API，提供丰富的动漫资源和类型筛选
- 🔗 **自定义豆瓣代理** - 自定义豆瓣API 代理和图片代理，防止连接问题
...
│   └── ...
├── lib/                     # 工具函数和 API
│   ├── doubanApi.js        # 豆瓣API 封装（电影/电视剧/动漫/综艺）
│   └── util.js             # 通用工具函数
└── store/                   # Zustand 状态管理
...
- **固定标签** (31 个):
  - 📊 推荐分类：热门、最新、豆瓣高分
  - 🎭 类型分类：动画、剧情、喜剧、动作、爱情、科幻、奇幻、冒险、悬疑、惊悚、恐怖、搞笑、校园、治愈、励志、热血、机战、魔法、神话、运动、音乐、舞蹈、美食、历史、战争、传记、纪录片
- **数据来源**: 豆瓣API
- **标签管理**: 支持自定义添加标签
