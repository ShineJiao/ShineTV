...
export async function fetchBangumiDaily(day) {
  try {
    console.log("正在获取 Bangumi 每日放送:", day);
    // Bangumi 每日放送 API
    const response = await fetch(`https://api.bgm.tv/calendar/${day}`);
    
    console.log("Bangumi API 响应状态:", response.status);
    ...
    console.log("Bangumi 每日放送数据:", data);
    ...
    console.log("转换后的 Bangumi 数据数量:", result.length);
    return result;
  } catch (error) {
...
export async function fetchBangumiRanking(type = 'tv', limit = 20, offset = 0) {
  try {
    console.log("正在获取 Bangumi 排行:", { type, limit, offset });
